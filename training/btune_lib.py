...
def load_data_chunk(
    ...
    @param files: list[str]
        Which measurements data subdirectories to get.
...
def normalize_train_test(train, test, meta_path=None, categories=None):
    ...
    `test` data. The `tradeoff` values are not normalized.
    @param train: pandas.DataFrame
        A data frame with 3 possible columns names: `cratio`, `speed` and `tradeoff`.
    ...
    if meta_path is not None:
        ...
        if categories is not None:
            ...
            filters = {
                'nofilter': blosc2.Filter.NOFILTER.value,
                'shuffle': blosc2.Filter.SHUFFLE.value,
                'bitshuffle': blosc2.Filter.BITSHUFFLE.value,
                'shuffle-bytedelta': blosc2.Filter.BYTEDELTA.value,
            }
            BLOSC_ALWAYS_SPLIT = blosc2.SplitMode.ALWAYS_SPLIT.value
            BLOSC_NEVER_SPLIT = blosc2.SplitMode.NEVER_SPLIT.value
    ...
    train_tradeoff = pd.DataFrame(train.tradeoff).reset_index(drop=True)
    test_tradeoff = pd.DataFrame(test.tradeoff).reset_index(drop=True)

    train_df = pd.concat([train_cratio, train_speed, train_tradeoff], axis=1)
    train_df = train_df.astype('float32')
    test_df = pd.concat([test_cratio, test_speed, test_tradeoff], axis=1)
...
def normalize_test(test, meta_dict):
    ...
    df = pd.concat([test_cratio, test_speed, test_tradeoff], axis=1)
    df = df.astype('float32')
...
def get_labels_row(dataframes, tradeoff=0.6, cspeed=True, errors=[0.05, 0.1, 0.2]):
    """
    ...
    for i in range(ratios.shape[1]):
        scores.insert(i, i, tradeoff * ratios.iloc[:, i] + (1 - tradeoff) * speeds.iloc[:, i])
...
def get_labels_tradeoffs(codecs_dfs, tradeoffs_array, cspeed=True, errors=[0.05, 0.1, 0.2]):
    """

    @param codecs_dfs: list[pandas.DataFrame]
        The list with the real measurements dataframes.
    @param tradeoffs_array: np.array(float)
        The list of tradeoffs to get the bests categories.
    @param cspeed: bool
        @see get_labels_row
    ...
    """
    bests_list = [pd.DataFrame()] * len(tradeoffs_array)
    for i in range(len(tradeoffs_array)):
        bests_list[i] = get_labels_row(codecs_dfs, tradeoffs_array[i], cspeed, errors)
...
def get_nn_input(probes_dfs, tradeoffs_array):
    """
    ...
    @param tradeoffs_array: np.array(float)
        Tradeoffs to train for.
    @return: pandas.DataFrame
        The entropy data to use plus the tradeoff for each row.
    """
    tradeoffs = pd.Series(tradeoffs_array).repeat(len(probes_dfs[0]))
    tradeoffs = tradeoffs.reset_index(drop=True)
    tradeoffs = tradeoffs.rename("tradeoff")

    probes = pd.concat([df for df in probes_dfs], axis=1)
    list_probes = [probes] * len(tradeoffs_array)
    nn_input = pd.concat(list_probes, axis=0)
    nn_input = nn_input.reset_index(drop=True)
    nn_input = pd.concat([nn_input, tradeoffs], axis=1)
...
def split_data(entropy, bests, ncategories):
    ...
    @param entropy: pandas.DataFrame
        ...
        (two columns for each probe: `cratio` and `speed`; plus the `tradeoff`).
    ...
    train_data = train[["cratio", "speed", "tradeoff"]]
    train_three_best = train.drop(['cratio', 'speed', 'tradeoff'], axis=1)
    test_labels = test['best']
    test_data = test[["cratio", "speed", "tradeoff"]]
    test_three_best = test.drop(['cratio', 'speed', 'tradeoff'], axis=1)
...
def most_predicted(preds, tradeoffs, categories, codecs_dfs):
    """
    Print a summary of the 3 most predicted categories
    and compute the mean cratio, cspeed and dspeed for the
    most predicted category for each tradeoff.
    @param preds: pandas.DataFrame
        Predictions done by the neural network.
    @param tradeoffs: pandas.Series
        The tradeoff corresponding to each prediction.
    @param categories: list[str]
    ...
    tradeoff_values = tradeoffs.unique()
    tradeoff_values.sort()
    for tradeoff in tradeoff_values:
        df = preds[tradeoffs == tradeoff]
        ...
        # Get most predicted category mean cratio, cspeed and dspeed for each tradeoff
    ...
    table.index = tradeoff_values

    return table


def scores_summary(preds, bests, tradeoffs):
    """
    Print a score summary for each tradeoff.
    @param preds: pandas.DataFrame
    ...
    @param tradeoffs: pandas.Series
        The tradeoff corresponding to each row from the other params.
    @return: None
    """
    print("Get score separately for each tradeoff")
    ranges = bests.columns.values.tolist()[3:]

    preds.columns = ["pred"]
    aux_df = pd.concat([preds, bests, tradeoffs], axis=1)
    ...
    for i in range(len(tradeoff_values)):
        df = aux_df[aux_df.tradeoff.isin([tradeoff_values[i]])]
